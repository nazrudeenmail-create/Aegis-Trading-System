...
The schema assumes a single broker and single account in Version 1. The design does not block multi-broker or multi-account expansion, but it does not add those columns now. When Phase 8 introduces a concrete broker, the schema can be extended without restructuring.
...
2. Create all 14 tables in dependency order
...
Tests will verify:
- All 14 tables are created successfully
